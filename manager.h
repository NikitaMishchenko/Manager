...
protected:
public:
    std::vector <std::string> commands; ///commands for managing
    std::vector <std::string> proceded_commands; /// managed commands
    ...
    void manage_command_index(int index_command); /// выполнение конкретной команды
...
///APPROXIMATINON
//void procedure_orthogonal_polynomial_curve_fitting(matrix m, const int index_x, const int index_y, const int polynom_degree, std::vector<double> &polynom);///fit y(x) as polynom
//void procedure_orthogonal_polynomial_curve_fitting(matrix m, const int index_x, const int index_y, const int index_w, int polynom_degree, std::vector<double> &polynom);///fit y(x) as polynom
...
void manager_get_cut_redd_results(std::string s);
void redd_top_bot_merge(std::string Data_Way, std::string base_file_name);
...
matrix procedure_get_pendulum_frequency0(std::string Data_Way, std::string base_file_name, oscillation D, std::string method_find_freq,
                                        int window_size, int step_size, int zero_index, int K_power, double avg_angle);///time // freq ///f = 1/T


matrix procedure_get_pendulum_frequency0(std::string Data_Way, std::string base_file_name, oscillation D, std::string method_find_freq,
                                        int window_size, int step_size, int zero_index, int K_power);///time // freq ///f = 1/T
...
void Redd_Cut_file(std::string Data_Way_instruct, std::string instruct_name, std::string Data_Way_out);
