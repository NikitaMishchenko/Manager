...
manager::manager()
{
    ...
    stop_command = "STOP";
    name = "";
    std::vector <std::string> n_c;
    std::vector <std::string> n_pc;
    std::vector <std::string> n_dp;
        commands = n_c;
...
{std::cout << "manager::manage_commands()" << std::endl;
    for (auto c : commands)
    {
        ///выполнение комманд
        std::cout << c << "\t";

        if( c != stop_command)///если не было фэйла при выполнении команды
        {
            sucess = true;
...
{std::cout << "manager::check_proceded_commands() = ";
...
{std::cout << "bool manager::reproceedable()" << std::endl;
    ...
    {//cout << "\tif\t" << commands.size() << "\t" << proceded_commands.size() << endl;
        std::vector<std::string>::const_iterator it_c = commands.begin(), it_pc = proceded_commands.begin();
        ...
        std::cout << "WARNING commands.size() > proceded_commands()" << commands.size() << "\t" << proceded_commands.size() << std::endl;
        return false;
    }
    std::cout << "WARNING smth happen false returned\n";
    return false;
}

///GET/SET/ADD/ERASE
void manager::set_commands(std::vector<std::string> new_commands)
{
    commands = new_commands;
    sucess = this->check_proceded_commands();
};

void manager::push_command(std::string new_command)
{
    commands.push_back(new_command);sucess = false;
    sucess = this->check_proceded_commands();
};

void manager::set_proceded_commands(std::vector<std::string> new_p_commands)
{
    proceded_commands = new_p_commands;
    sucess = this->check_proceded_commands();
};
void manager::push_proceded_command(std::string new_p_command)
{
...
///FILES
void manager::Load_manager(std::string full_file_name)
{std::cout << "void manager::Load_manager(string full_file_name)" << std::endl;
    std::ifstream fin(full_file_name + manager_file_format);
        std::string buff;
        fin >> buff;
...
void manager::Save_manager(std::string full_file_name)
{
    std::ofstream fout(full_file_name + manager_file_format);
        fout << mark_file_manager_begin << std::endl;
...
///EXTRA
std::string manager::get_info()
{
    std::string all_commands,all_proceded_commands,all_data_paths;
    for (auto c : commands)
            all_commands += (c + "\t");
    for (auto c : proceded_commands)
    ...
    for (auto c : data_paths)
            all_data_paths += (c + "\t");
    return "Manager object\t sucess=" + my_to_string(sucess) + "\t"
                                            + std::to_string(static_cast<uint32_t>(commands.size()))
                                                + "\t" + std::to_string(static_cast<uint32_t>(proceded_commands.size()))
                                                + "\n\tcommands:\t" + all_commands + "\n\tproceded_commands:\t" + all_proceded_commands + "\n\tdata_paths:\t" + all_data_paths + "\n";
}
void manager::info()
{
    std::cout << "Manager object\n";
        std::cout << "sucess commands = " << sucess << "\t" << commands.size() << "\t" << proceded_commands.size() << std::endl;
        std::cout << "\tstored commands:\t";
        for (auto c : commands)
            std::cout << c << "\t";
        std::cout << "\n\tproceded_commands:\t";
        for (auto c : proceded_commands)
...
{
    std::cout << "Manger load file info" << std::endl;
    std::cout <<  "\tbegins with \"" << mark_file_manager_begin << "\""<< std::endl;
    std::cout <<  "\tcommands load after \"" << mark_file_commands << "\"" << std::endl;
    std::cout <<  "\tproceded_commands load after \"" << mark_file_proceded_commands << "\"" << std::endl;
    std::cout <<  "\tends with \"" << mark_file_manager_end << "\"" << std::endl;
    std::cout << "WARNING data must end by symbol" << std::endl;
}

void manager::create_load_from_file_fishfile(const std::string full_file_name)
{
    std::ofstream fout(full_file_name + manager_file_format);
        fout << mark_file_manager_begin << std::endl;
        fout << mark_file_commands << "\t***enter_commands_instead***" << std::endl;
        fout << mark_file_proceded_commands << "\t***enter_proceded_commands_instead***" << std::endl;
        fout << mark_file_data_paths << "\t***enter_data_paths_instead***" << std::endl;
        fout << mark_file_manager_end << std::endl;
    fout.close();
}
...
double Load_marked_double_from_file(std::string Data_Way, std::string file_name, std::string mark, int index)
{//cout << "Load_marked_double_from_file()\n";
   // if(FileExists( (Data_Way + file_name) ){
        std::ifstream fin(Data_Way + file_name);
        std::string s_buff = "";
        double d_buff = 0;
        while(!fin.eof())
        {
...
double Load_marked_double_from_file(std::string Data_Way, std::string file_name, std::string mark)///index = 1
...
/*
///APPROXIAMTION
 void procedure_orthogonal_polynomial_curve_fitting(matrix Data, const int index_x, const int index_y, const int index_w, const int polynom_degree, std::vector<double> &polynom)
 {
    std::vector<double> x(Data.get_height());
    std::vector<double> y(Data.get_height());
    std::vector<double> w(Data.get_height());

    if(index_w >= 0)
        for(int i = 0; i < Data.get_height(); ++i)
        {
            x[i] = Data.get_data(i, index_x);
            y[i] = Data.get_data(i, index_y);
            w[i] = 1.0;
        }
    else
        for(int i = 0; i < Data.get_height(); ++i)
        {
            x[i] = Data.get_data(i, index_x);
            y[i] = Data.get_data(i, index_y);
            w[i] = Data.get_data(i, index_w);
        }

    Ortho_GLS fitter;
        fitter.recurrent_P(polynom_degree,x,y,polynom);
 };
...
matrix collect_dynamic_coefficients(matrix* M_arr, int M_arr_length)
{
    matrix result;
    for(int i = 0; i < M_arr_length; i++)
        result = result.merge_height(M_arr[i]);
    result = result.sort_by_column_index(1);///angle
    return result;
}

matrix collect_top_bot(std::string Data_Way, std::string base_name, std::string postfix)
{
    matrix result;
    matrix m;
        m.Load_matrix(Data_Way+base_name+"_Redd_bot"+postfix+".txt");
        result = result.merge_height(m);
        m.Load_matrix(Data_Way+base_name+"_Redd_top"+postfix+".txt");
        result = result.merge_height(m);
    return result;
}

void procedure_make_dynamic_coefficient_collection(std::string instruction_file_name)
{
    ifstream fin(instruction_file_name + ".txt");
        std::string Data_Way = "C:/Base/Proc_Exp/388-s2/M2.25/CUT/Cut_results/";
            fin >> Data_Way;
        ...
        std::string* S_arr = new string [D_counter];
        for(int i = 0; i < D_counter; i++)
            fin >> S_arr[i];
    fin.close();


    matrix* m = new matrix [D_counter];
    string c_name;
    for(int i = 0; i < D_counter; i++)
    {
        c_name += (S_arr[i]+"_");
        m[i] = collect_top_bot(Data_Way, S_arr[i], postfix);
    }
    collect_dynamic_coefficients(m, D_counter).Save_matrix(Data_Way + c_name+postfix+".txt");

}

matrix prep_collection(matrix collection, int index)
{
    matrix result;
    ///add same dots to balance weights
...
void manager_get_cut_redd_results(string instructions_name)
{
    string buff;
    ifstream fin(instructions_name);
        fin >> buff;
    fin.close();
}

void redd_top_bot_merge(std::string Data_Way, std::string name)
{
    matrix m1,m2;
    //std::string name = "4217_2";//"C:/Base/Proc_Exp/388-s2/M2.25/CUT/Cut_results/"
    m1.Load_matrix(Data_Way +name + "_Redd_bot_0.txt");
        m2.Load_matrix(Data_Way + name + "_Redd_top_0.txt");
    m1 = m2.merge_height(m1);
        m1.Save_matrix(Data_Way + name + "_Redd_BOTH_0.txt");

    m1.Load_matrix(Data_Way +name + "_Redd_bot_3.txt");
        m2.Load_matrix(Data_Way + name + "_Redd_top_3.txt");
    m1 = m2.merge_height(m1);
        m1.Save_matrix(Data_Way + name + "_Redd_BOTH_3.txt");

    m1.Load_matrix(Data_Way +name + "_Redd_bot_10.txt");
        m2.Load_matrix(Data_Way + name + "_Redd_top_10.txt");
    m1 = m2.merge_height(m1);
        m1.Save_matrix(Data_Way + name + "_Redd_BOTH_10.txt");
}
...
void Fix_LIR_Data0(string Data_Way, string base_file_name, string result_file, double border) ///border - значение разрыва углов котрое считается неприемлимым
{
    matrix A1;
    ...
    for(int i = A1.get_height()-2; i >= 0; --i)
    {//cout << i << "\t" ;
        gap = (A1.get_data(i+1,1)) - (A1.get_data(i,1));
        if(gap >= border )
            {flag = true;//cout << "true\n";
            }
        if(gap < border )
            {flag = false;//cout << "false\n";
            }

        if(flag==true )
            A1.set_data(i,1, A1.get_data(i,1) + 360.0);
    }
    //A2.info();
   // A1 = A1.m_column_shift(1, -A1.get_data(A1.get_height()-1, 1));
    A1.Save_matrix(Data_Way + result_file + ".txt");
}

void Fix_LIR_Data(string Data_Way, string base_file_name, string result_file, double border)
{
    matrix D;
        D.Load_matrix(Data_Way + base_file_name + ".txt");

    for(int i = D.get_height()-2; i >=0; --i)
    {
        if(fabs(D.get_data(i,1) - D.get_data(i+1,1))>10){
            D.set_data(i, 1, D.get_data(i,1) + (D.get_data(i,1)<= 0 ? 1.0 : -1.0)*360.0);
        }
    }
    D = D.m_column_shift(1, -D.get_data(D.get_height()-1, 1));
    D.Save_matrix(Data_Way + result_file + ".txt");
}
...
matrix procedure_get_pendulum_frequency0(string Data_Way, string base_file_name, oscillation D, string method_find_freq, int window_size, int step_size, int zero_index, int K_power, double avg_angle)///time // freq ///f = 1/T
{cout << "angle_changing\t" << avg_angle << endl;
    ...
    if(avg_angle)
        a.m_column_shift(0, -1.0*avg_angle);
            D.set_angle(a);
    return procedure_get_pendulum_frequency0(Data_Way,  base_file_name, D,  method_find_freq, window_size, step_size, zero_index, K_power);///time // freq ///f = 1/
}


matrix procedure_get_pendulum_frequency0(std::string Data_Way, std::string base_file_name, oscillation D, string method_find_freq, int window_size, int step_size, int zero_fit_index, int K_power)///time // freq ///f = 1/T
{std::cout << "procedure_get_pendulum_frequency()\n";

    std::string pendulum_name = "_pendulum";
    ...
    if(true)
    {
        if (method_find_freq == "FFT")
        {
            ...
            ///11.10.2020
            f_m = Get_Freq_Window_FFT(dat, step_size, window_size, zero_fit_index);
//            f_m = get_freq_window_step(dat, step_size, window_size, zero_fit_index);
...
{cout << "procedure_ampl_analisys(string Data_Way, string base_file_name, oscillation D, double freq)\n";
    ...
    ///TEST
        ...
        ofstream fout_names(Data_Way + "Saved_Files_names_" + base_file_name + ".txt");
            fout_names << base_file_name + "_env_top" << endl;
            fout_names << base_file_name + "_env_bot" << endl;
            fout_names << base_file_name + "_env_tb" << endl;
            fout_names << base_file_name + "_env_top_sm3" << endl;
            fout_names << base_file_name + "_env_bot_sm3" << endl;
            fout_names << base_file_name + "_env_tb_sm3" << endl;
            fout_names << base_file_name + "_env_top_sm10" << endl;
            fout_names << base_file_name + "_env_bot_sm10" << endl;
            fout_names << base_file_name + "_env_tb_sm10" << endl;
            fout_names << base_file_name + "_env_top_redd" << endl;
            fout_names << base_file_name + "_env_bot_redd" << endl;
            fout_names << base_file_name + "_env_top_sm3_redd" << endl;
            fout_names << base_file_name + "_env_bot_sm3_redd" << endl;
            fout_names << base_file_name + "_env_top_sm10_redd" << endl;
            fout_names << base_file_name + "_env_top_sm10_redd" << endl;
            fout_names << base_file_name + "_env_tb_redd" << endl;
            fout_names << base_file_name + "_env_tb_sm3_redd" << endl;
            fout_names << base_file_name + "_env_tb_sm10_redd" << endl;
            fout_names << base_file_name + "_middle_angle" << endl;
        fout_names.close();
...
void manager_res_simple_cut(string Data_Way, string new_name, string file_type, string out_name, string new_file_type, double time_start, double time_end)
{
    matrix A;
        if(FileExists(Data_Way + new_name + file_type + ".txt"))
        {
            A.Load_matrix(Data_Way + new_name + file_type + ".txt");//A.info();cout<< A;
                data_cutter(A, 0, time_start, time_end).Save_matrix(Data_Way + out_name + new_file_type + ".txt");
        }
}

void Redd_Cut_file(string Data_Way_instruct, string instruct_name, string Data_Way_out)
{
    string new_name, out_name = "";
    //matrix A;
    double time_end = 0, time_start = 0;
if(FileExists(Data_Way_instruct + instruct_name + ".txt"))
{

    ifstream fin(Data_Way_instruct + instruct_name + ".txt");
        //cout << Data_Way_instruct + instruct_name + ".txt" << endl;
    ...
    while(new_name != "END")
    {
        ...
        manager_res_simple_cut(Data_Way_out, new_name, "_env_top_redd", out_name, "_Redd_top_0", time_start, time_end);
        manager_res_simple_cut(Data_Way_out, new_name, "_env_bot_redd", out_name, "_Redd_bot_0", time_start, time_end);
        manager_res_simple_cut(Data_Way_out, new_name, "_env_tb_redd", out_name, "_Redd_tb_0", time_start, time_end);

        manager_res_simple_cut(Data_Way_out, new_name, "_env_top_sm3_redd", out_name, "_Redd_top_3", time_start, time_end);
        manager_res_simple_cut(Data_Way_out, new_name, "_env_bot_sm3_redd", out_name, "_Redd_bot_3", time_start, time_end);
        manager_res_simple_cut(Data_Way_out, new_name, "_env_tb_sm3_redd", out_name, "_Redd_tb_3", time_start, time_end);

        manager_res_simple_cut(Data_Way_out, new_name, "_env_top_sm10_redd", out_name, "_Redd_top_10", time_start, time_end);
        manager_res_simple_cut(Data_Way_out, new_name, "_env_bot_sm10_redd", out_name, "_Redd_bot_10", time_start, time_end);
        manager_res_simple_cut(Data_Way_out, new_name, "_env_tb_sm10_redd", out_name, "_Redd_tb_10", time_start, time_end);
...
cout << "NO FILE TO CUT\n" << Data_Way_instruct + instruct_name + ".txt" << endl;
}
}
...
void get_mz_results(string Data_Way, string base_name, string result_name, int from_index, int to_index)
{
    ...
    for(int angle_num = from_index; angle_num <= to_index; angle_num++) ///по карте читать чтобы дважды не вызывать FileExists
        ...
        if(FileExists((Data_Way + "/" + manager_int_num(angle_num) + "degree/" + base_name).c_str()))///no format
        {
            ...
            //fin.open(Data_Way + "/" + manager_int_num(angle_num) + "degree/" + base_name);
            for(int j = 0; j < fh; j++)
            {
